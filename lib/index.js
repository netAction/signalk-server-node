...
Server.prototype.start = function() {
  ...
  this.app.pruneInterval = setInterval(
    app.signalk.pruneContexts.bind(app.signalk, 60 * 60),
    60 * 1000
  )
  this.app.providers = []
...
function createServer(app, cb) {
  if (
    typeof app.config.settings.ssl === 'undefined' ||
...
function getCertificateOptions(cb) {
  if (
    fs.existsSync('./settings/ssl-key.pem') &&
    fs.existsSync('./settings/ssl-cert.pem')
  ) {
    if (!hasStrictPermissions(fs.statSync('./settings/ssl-key.pem'))) {
      cb(
        new Error(
          './settings/ssl-key.pem must be accessible only by the user that is running the server, refusing to start'
        )
      )
      return
    }
    if (!hasStrictPermissions(fs.statSync('./settings/ssl-cert.pem'))) {
      cb(
        new Error(
          './settings/ssl-cert.pem must be accessible only by the user that is running the server, refusing to start'
        )
      )
      return
    }
