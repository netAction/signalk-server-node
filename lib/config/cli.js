...
module.exports = function (app) {
  ...
  function getSettingsFilename (argv) {
    ...
    if (app.overrides.settings && typeof app.overrides.settings === 'string') {
      debug(
        'Settings filename was set using .settings(), overriding all other options'
      )
  ...
  if (
    app.overrides.settings !== null &&
    typeof app.overrides.settings === 'object'
  ) {
    debug(
      'Settings object was set using .settings(), overriding all other options'
    )
  ...
  if (
    ...
    (typeof app.config.settings.vessel.uuid === 'string' ||
      typeof app.config.settings.vessel.mmsi === 'string')
  ) {
    app.selfType = 'mmsi'
